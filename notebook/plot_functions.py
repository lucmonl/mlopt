...
def plot_xlogy(ax, xaxis, yaxis, name, alpha=1.0, linewidth=1.0):
    line = ax.semilogy(xaxis, yaxis, alpha=alpha, linewidth=linewidth)[0]
    ax.set_xlabel('Epoch')
    ax.set_title(name)
    return line
...
def plot_attr(ax, train_graphs, attr, start=None, end=None, alpha=1.0, linewidth=1.0):
    data = []
    xaxis = None
    ...
    if attr in ['test_err']:
        ...
        line = plot_xlogy(ax, xaxis, yaxis, name=attr, alpha=alpha, linewidth=linewidth)
        ...
        #yaxis = getattr(train_graphs, attr)[start:end]
        line = plot_xy(ax, xaxis, yaxis, name=attr, alpha=alpha, linewidth=linewidth)
        ax.fill_between(xaxis, yaxis - stds, yaxis + stds, alpha=alpha, label="Confidence Interval")
    elif attr in ['loss_ratio']:
        line = plot_xy(ax, xaxis[:-1], yaxis, name=attr, alpha=alpha, linewidth=linewidth)
...
def plot_attr_overlap(ax, train_graphs, attr, start=None, end=None, alpha=1.0, linewidth=1.0):
...
def plot_figures_opts_attr_ci(opts_list, model_params, opt_params, attrs, start=None, end=None, alpha=1.0, linewidth=1.0, legend_fontsize=None, ylabel=None, legends=[], titles=[], yaxis=[], xlabels=[], save_dir=None, return_last=False, return_max=False, overlap=False):
    import matplotlib.ticker as mtick
    rows, cols = 1, len(opts_list)
    fig, axs = plt.subplots(rows, cols, figsize=(cols*4, rows*3))
    last_val = []
    ...
    for opts, legend, title, attr, xlabel in zip(opts_list, legends, titles, attrs, xlabels):
        ...
        for opt_name in opts:
            ...
            line = plot_fn(ax=axs[ax_ptr], train_graphs=train_graphs, attr=attr, start=start, end=end, alpha=alpha, linewidth=linewidth)
