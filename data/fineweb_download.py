...
DATA_CACHE_DIR = os.path.join(os.environ["DATA_HOME"], local_dir)
...
# 2. Extract the necessary vocabulary and special tokens
# Llama 3 uses a Tiktoken tokenizer internally
enc = hf_tokenizer.backend_tokenizer

#eot = enc._special_tokens['<|eot_id|>'] # end of text token
eot = enc.encode('<|eot_id|>').ids[0]
def tokenize(doc):
    # tokenizes a single document and returns a numpy array of uint16 tokens
    tokens = [eot] # the special <|endoftext|> token delimits all documents
    #tokens.extend(enc.encode_ordinary(doc["text"]))
    tokens.extend(enc.encode(doc["text"]).ids)
