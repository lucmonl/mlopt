...
def add_adapters_homo(client_num, model_name, model, lora_rank, lora_alpha, opt_params, lora_freeze_a=False):
    ...
    if opt_params["fedlora_avg"] in ["fr", "fr_v2", "muonlora_v1", "muonlora_v2", "muonlora_v3", "muonlora_v4", "muonlora_v5", "muonlora_v6", "muonlora_v7","muonlora_v8"]:
    ...
    if opt_params["fedlora_avg"] == "svd":
        truncate_last=False
    elif opt_params["fedlora_avg"] in ["avg", "flora", "flasc", "fr", "fr_v2", "muonlora_v1", "muonlora_v2", "muonlora_v3", "muonlora_v4", "muonlora_v5", "muonlora_v6", "muonlora_v7", "muonlora_v8"]:
        truncate_last=True
    else:
        assert False
    ...
    if opt_params["fedlora_avg"] in ["muonlora_v1", "muonlora_v2", "muonlora_v3",  "muonlora_v4", "muonlora_v5", "muonlora_v6", "muonlora_v7", "muonlora_v8"]:
        # use the name fr_save_neg_init to avoid subsequent if conditions on  $"fr_save_init" in name$                                                    
        model, output_layer_name, Lora_config = add_adapters_dataset(model_name, model, client_rank, lora_alpha, \
...
def add_adapters(model, lora_rank, lora_alpha, output_layer_name, target_modules, server_name, freeze_a=False, task_type=None, adapter_name="default", init_lora_weights=True):
    ...
    if lora_rank > 0:
        ...
        if server_name == adapter_name:
            ## make sure it's the first time add_adapter is called
            assert not isinstance(model, LoraModel)
            model = LoraModel(model, config, adapter_name=adapter_name)
        else:
            #print(adapter_name)
