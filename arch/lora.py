...
def add_adapters_homo(client_num, model_name, model, lora_rank, lora_alpha, opt_params, lora_freeze_a=False):
    ...
    if opt_params["fedlora_avg"] in ["fr", "muonlora_v1", "muonlora_v2", "muonlora_v3", "muonlora_v4"]:
    ...
    if opt_params["fedlora_avg"] == "svd":
        truncate_last=False
    elif opt_params["fedlora_avg"] in ["avg", "flora", "flasc", "fr", "muonlora_v1", "muonlora_v2", "muonlora_v3", "muonlora_v4"]:
        truncate_last=True
    else:
        assert False
    ...
    if opt_params["fedlora_avg"] in ["muonlora_v1", "muonlora_v2", "muonlora_v3",  "muonlora_v4"]:
        # use the name fr_save_neg_init to avoid subsequent if conditions on  $"fr_save_init" in name$                                                    
        model, output_layer_name, Lora_config = add_adapters_dataset(model_name, model, client_rank, lora_alpha, \
