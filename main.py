...
if __name__ == "__main__":
    ...
    parser.add_argument("--ef21_lmo", type=str, default="svd", choices=["ns5", "svd"], help="ef14muon/ef21muon: how the spectral-norm LMO of the server step is evaluated. 'ns5' is the quintic Newton-Schulz iteration used for training; 'svd' is the exact U V^T, an fp32 reference that costs a full SVD per trainable matrix per round (~2s each for 3072x3072, i.e. minutes per round for a 3B model) and is meant for measuring how far the polynomial iteration is from the true orthogonal factor")
    ...
    if apply_lora:
        ...
        if opt_params["opt_name"] == "federated":
            ...
            if opt_params["fedlora_avg"] == "muonlora_v18":
                ...
                # No alternation schedule here, so any switch interval passed is
                # inert and stays out of the path.  The factor step is still
                # server_lr * merge_alpha, so that has to name the run or two
                # factor learning rates share a directory.  Tagged only when on,
                # so alternating runs keep their paths.
                model_params = model_params | {"muonlora_factors": "both",
                                               "muonlora_merge_alpha": args.muonlora_merge_alpha}
