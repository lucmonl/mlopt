...
def transformer_probe(graphs, model, train_loader, test_loader, device, zero_out_attn=-1):
    logisticRegr = LogisticRegression()
    
    cls_tokens, targets = model.get_cls_tokens(train_loader, device, zero_out_attn)
    cls_tokens_test, targets_test = model.get_cls_tokens(test_loader, device, zero_out_attn)
    graphs.layer_cls_score[zero_out_attn] = []
